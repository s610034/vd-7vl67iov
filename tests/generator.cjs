...
const data={customerName:'test',sshUser:'test',sshPass:'',clusterKey:'a'.repeat(32),wazuhApiPassword:'Quote"\\Dollar$`\'!# secret',vip:'10.0.0.100',vipCidr:'24',dnsServer:'10.0.0.1',gateway:'10.0.0.1',platform:'pve',opensearchPass:'test-only',needsVip:true,idx:[host('idx','10.0.0.10')],mgr:[host('master','10.0.0.20'),host('worker','10.0.0.21')],dash:[host('dash','10.0.0.30')],lb:[host('lb1','10.0.0.40'),host('lb2','10.0.0.41')],nids:[],allInOne:{enabled:false}};
...
for(const lb of [[],data.lb.slice(0,1),data.lb]){
 ...
 const inj=c.buildInjectDashboardScript(d);
 assert(inj.includes('url: https://10.0.0.20'));
 assert(inj.includes(JSON.stringify(data.wazuhApiPassword)));
 const sop=c.buildSopManual(d);
 assert(sop.includes('url: https://10.0.0.20'));
 assert(!sop.includes('<interval>'));
}
...
write('03-inject-manager.sh',c.buildInjectManagerScript(data));
write('04-inject-dashboard.sh',c.buildInjectDashboardScript(data));
write('05-inject-lb.sh',c.buildInjectLbScript(data));
const lb=c.buildInjectLbScript(data);
assert(lb.includes('bind *:1515'));assert(lb.includes('bind *:55000'));
assert(lb.includes('server master 10.0.0.20:55000'));
